    ...
    <?php foreach($form as $id => $f): ?>
      ...
      <?php if(!$f->isHidden()): ?>
        ...
        <tr id="tr_reply_author_delete_<?php echo $form->getName() ?>" class="tr_reply_author_delete">
          <td><?php echo link_to_function(__("Delete the reply", array(), 'vjComment'), "deleteReply('".$form->getName()."')", array('class' => 'delete_reply'))."\n" ?></td>
        </tr>
        <?php endif ?>
      <?php endif ?>
    <?php endforeach ?>
        <tr>
          ...
          <button type="submit" class="submit button1"><?php echo __('send', array(), 'vjComment') ?></button>
          </td>
        </tr>
      </table>
  </form>
